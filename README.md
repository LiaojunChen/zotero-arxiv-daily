...
reranker:
  ...
  siliconflow:
    key: null # SiliconFlow API key for the rerank endpoint. Example: sk-xxx
    url: https://api.siliconflow.cn/v1/rerank # SiliconFlow rerank endpoint.
    model: Qwen/Qwen3-Reranker-0.6B # The rerank model name.
    batch_size: 64 # Number of candidate papers per rerank request.
    max_query_papers: 30 # Number of recent Zotero papers used to build the interest query.
    max_query_chars: 12000 # Character budget for the interest query.
    max_document_chars: 4000 # Character budget per candidate document.
    score_scale: 10.0 # Multiplier applied to relevance_score for email display.
    timeout: 60 # Request timeout in seconds.
    instruction: "Given a research-interest profile, rank candidate papers by relevance and usefulness to the user's research."
...
executor:
  ...
  reranker: local # The reranker to use. Example: 'local', 'api' or 'siliconflow'
```
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. With `executor.reranker: siliconflow`, it builds a research-interest query from your recent Zotero papers and sends candidate paper titles/abstracts to the SiliconFlow rerank endpoint. The returned `relevance_score` is used to sort the candidate papers. The older `local` and `api` rerankers still calculate embedding similarity between candidate abstracts and Zotero abstracts. The TLDR of each paper is generated by LLM, given the text extracted by pymupdf4llm.
